...
/**
 * Visit interface for the {@link Regexp} hierarchy.
 * 
 * @author <a href="http://haumacher.de">Bernhard Haumacher</a>
 */
public interface RegexpVisitor<R, A> {

	R visit(RegexpAlphabet expr, A arg);
	R visit(RegexpAlternative expr, A arg);
	R visit(RegexpSequence expr, A arg);
	R visit(RegexpIdentifier expr, A arg);
	R visit(RegexpString expr, A arg);
	R visit(RegexpOptional expr, A arg);
	R visit(RegexpOneOrMore expr, A arg);
	R visit(RegexpZeroOrMore expr, A arg);
